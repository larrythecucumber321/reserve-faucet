...
import MultiSendInterface from "./MultiSendInterface.json";
...
const BLOCK_FAUCET_DRIPS_TIMEOUT = 1 * 1000; // 1 seconds
...
export default class EVM {
  ...
  async sendToken(
    ...
    try {
      ...
      await asyncCallWithTimeout(
        ...
        PENDING_TX_TIMEOUT,
        `Timeout reached for transaction with nonce ${this.nonce}`
      );
  ...
  async fetchERC20Balance(): Promise<void> {
    this.contracts.forEach(async (contract: any) => {
      ...
      contract.balance = new BN(balance);
    });
  }
  ...
  async putInQueue(req: RequestType): Promise<void> {
    ...
    if (this.balanceCheck(req)) {
      ...
      this.requestCount--;
      this.log.warn(
        "Faucet balance too low! " + req.id + " " + this.getBalance(req.id)
      );
      this.hasError.set(
        req.receiver,
